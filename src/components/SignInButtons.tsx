...
function Inner() {
  ...
  async function passkey() {
    ...
    if (r?.error) {
      setError("No passkey was used. Sign in with Google, then add a passkey in Settings.");
      setBusy(null);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <Button tone="yellow" size="lg" block onClick={google} disabled={busy !== null || isPending}>
      ...
      <Button size="lg" block onClick={passkey} disabled={busy !== null || isPending}>
        {busy === "passkey" ? "Waiting for passkey…" : "Sign in with a passkey"}
      </Button>
      {error && <p className="brut-sm rounded-[10px] bg-coral px-3 py-2 text-[13px] font-bold">{error}</p>}
    </div>
  );
}
